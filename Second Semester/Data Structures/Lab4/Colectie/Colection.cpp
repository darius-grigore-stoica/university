...
void Collection::add(TElem elem) {
    ...
    if (poz == -1) {
       ...
       this->occurrences.insert(value + 1, poz);
    }
}
...
Collection &Collection::operator=(const Collection &c) {
    ...
    for (i = 0; i < c.distinctelements; i++) {
        ...
        this->occurrences.insert(i, c.occurrences.getAt(i));
    }
    return *this;
}
    ...
    for (int i = 0; i < this->distinctelements - 1; ++i) {
        for (int j = i + 1; j < this->distinctelements; ++j) {
            if(this->elements.getAt(i) <= this->elements.getAt(j)) {
                ...
                this->elements.insert(i, this->elements.getAt(j));
                this->elements.insert(j, aux);
                this->occurrences.insert(i, this->occurrences.getAt(j));
                this->occurrences.insert(j, aux2);
            }
        }
    }
}
