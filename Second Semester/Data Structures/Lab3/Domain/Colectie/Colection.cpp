...
//    TElem *newElems = new TElem[this->capacity];
    VectorDinamic<TElem> newElems = VectorDinamic<TElem>(this->capacity);
    int *newOcc = new int[this->capacity];
    int i;
    for (i = 0; i < this->distinctelements; i++) {
//        newElems[i] = this->elements[i];
        newElems.update(i, this->elements.getAt(i));
...
Collection::Collection() {
    ...
    this->capacity = 10;
    this->elements = VectorDinamic<TElem>(this->capacity);
...
Collection &Collection::operator=(const Collection &c) {
    ...
    for (i = 0; i < c.distinctelements; i++) {
        this->elements.update(i, c.elements.getAt(i));//copierea elementelor din c in this
...
Collection::Collection(TElem *elements, int *occurences, int distinctelements, int capacity) {
    ...
    for(int i = 0; i < distinctelements; i++) {
        this->elements.update(i, elements[i]);
    ...
    for (int i = 0; i < this->distinctelements - 1; ++i) {
        for (int j = i + 1; j < this->distinctelements; ++j) {
            if(this->elements.getAt(i) <= this->elements.getAt(j)) {
                ...
                this->elements.update(i, this->elements.getAt(j));
                this->elements.update(j, aux);
