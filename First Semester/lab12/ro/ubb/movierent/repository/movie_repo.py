...
class MovieRepository:
    ...
    def add(self, movie: Movie):
        ...
        try:
            id = movie.id
            title = movie.title
            if self.findByTitle(title) is None:#daca nu exista deja un film cu titlul din movie
                if self.findById(id) is None:#atunci verificam ca id-ul sa nu existe deja
    ...
    def delete(self, id):
        ...
        try:
            if self.findById(id) is not None:
                ...
                for index in range(int(id) - 1, len(self._all_movies)):
                    self._all_movies[index].set_id(int(self._all_movies[index].id) - 1)
            else:
    ...
    def findById(self, id):
        ...
        for i in range(0, len(self._all_movies)):
            if self._all_movies[i].id == id:
                return self._all_movies[i].id
        return None
    ...
    def findByTitle(self, title):
        ...
        """
        for i in range(0, len(self._all_movies)):
            if self._all_movies[i].title == title:
                return self._all_movies[i].title
        return None

    def getMostWantedMovies(self):
        for index in range(0, len((self._all_movies))):
            for index2 in range(index, len(self._all_movies)):
                if int(self._all_movies[index].rented_times) < int(self._all_movies[index2].rented_times):
